...
const NavBar = () => {
    ...
    return (
        <nav className={`container mx-auto ${isMenuOpen ? 'menu-open' : ''}`}>
            <style>
                {`
                    /* Untuk mode gelap */
                    .dark .bg-white {
                    ...
                    /* Teks yang tetap hitam */
                    .nav-link {
                        color: #000000 !important; /* Tetapkan warna teks menjadi hitam */
                    }
                `}
            </style>
            <div className="flex justify-around">
                ...
                <div className="hidden md:flex items-center space-x-1">
                    {links.slice(0, 3).map(link => (
                        <a key={link.name} href={link.href} className={`py-5 px-3 text-zinc-700 dark:text-white hover:text-zinc-900 dark:hover:text-white ${link.name === 'Home' || link.name === 'Services' || link.name === 'Contact' ? 'nav-link' : ''}`}>{link.name}</a>
                    ))}
